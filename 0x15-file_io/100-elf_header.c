 ...
 * @e_ident:  pointer to array contain ELF magic number
 *
 * Description: If is not  ELF file - exit code 98.
 */
void check_elf(unsigned char *e_ident)
{
	int ind;
...
/**
 * print_magic - Print magic numbers of  ELF head
 * @e_ident: pointer to array contain ELF magic number
 *
...
int main(int __attribute__((__unused__)) argc, char *argv[])
{
	...
	re = read(on, header, sizeof(Elf64_Ehdr));
	if (re == -1)
	{
		free(header);
		close_elf(on);
